...
#  get_new_img - основная функция, которая возвращает tensor, т.е. итоговое изображение;
#  На вход она получает пути до исходных картинок и (опцонально) максимальный размер одного измерения изображения
def get_new_img(content_path, style_path, max_size=256):

    #  Опрелеяет размер изображений
    def get_size(content_path, max_size=None):
    ...
    imsize = get_size(content_path, max_size)
    ...
    def run_style_transfer(cnn, normalization_mean, normalization_std, content_img, style_img,
                           input_img, num_steps=500, style_weight=100000, content_weight=1):
